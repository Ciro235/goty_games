...
import { FaCalendarAlt, FaStar, FaGamepad, FaClock, FaUsers, FaQrcode, FaCrown, FaExpand, FaCompress } from "react-icons/fa";
...
export const GameCard: React.FC<GameCardProps> = ({ game, index }) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className={`relative overflow-hidden bg-gray-900 ${
          ...
          <div className="absolute top-4 right-4 z-20 flex gap-2">
            {/* Botón QR - SIEMPRE FUNCIONAL */}
            <button
              onClick={() => setShowQR(true)}
              className="p-2.5 bg-black/50 backdrop-blur-md rounded-full hover:bg-black/70 transition-all duration-300 hover:scale-110 shadow-lg hover:shadow-blue-500/30 group/qr"
              aria-label="Ver código QR"
              title="Ver código QR"
            >
              <FaQrcode className="text-white text-base group-hover/qr:text-blue-300 transition-colors duration-300" />
            </button>
      ...
      {/* Modal QR - FUNCIONAL */}
      {showQR && <QRModal game={game} onClose={() => setShowQR(false)} />}
    </>
  );
};